...
//        editor.putString("password", pass); // optional, only if needed
//        editor.apply();
//
//        // -------------------------
//        // Optionally, open ProfileActivity immediately
//        // -------------------------
...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void validateForm() {
        ...
        if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) { etEmail.setError("Enter a valid email address"); etEmail.requestFocus(); return; }
        ...
        // Phone
        if (phone.length() != 10) { etPhone.setError("Phone number must be 10 digits"); etPhone.requestFocus(); return; }

        // OTP
        if (otp.isEmpty()) { etOtp.setError("OTP is required"); etOtp.requestFocus(); return; }
        if (otp.length() != 6) { etOtp.setError("OTP must be 6 digits"); etOtp.requestFocus(); return; }
